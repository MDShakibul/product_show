...
import { useSearchCategoriesQuery, useGetCategoriesQuery } from '../lip/service/categoriesApi.js'

export default function CategoryPicker({ value, onChange, placeholder = 'Search category…' }) {
  ...
  const { data: suggestions, isFetching: searching } =
    useSearchCategoriesQuery(term, { skip: !searchingEnabled })
  const { data: firstPage } =
    useGetCategoriesQuery({ offset: 0, limit: 10 }, { skip: !listingEnabled })
  ...
  return (
    <div className="relative" ref={containerRef} onKeyDown={onKeyDown}>
      ...
      >
        ...
        <svg aria-hidden="true" className="h-4 w-4 ml-2 shrink-0" viewBox="0 0 20 20" fill="currentColor">
  ...
  >
    <div className="p-2">
      <input
        autoFocus
        // override your `.input` bg which was dark
    ...
    {items?.length ? items.map((c, idx) => (
      <button
        ...
        className={`w-full text-left px-3 py-2 rounded-lg hover:bg-black/5 ${idx === highlight ? 'bg-black/5' : ''}`}
